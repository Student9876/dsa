...
void helper(TN* root, int key, TN** successor){
    if(root == NULL) return;
    if(root->val < key) helper(root->right, key, successor);
    else {
        if((*successor) == NULL or (*successor)->val > root->val and root->val > key) (*successor) = root;
        helper(root->left, key, successor);
    }
}

// Recursion
TN* findInorderSuccessor(TN* root, int key){
    TN* successor = NULL;
...
// Iterative
TN* findInorderSuccessor_(TN* root, int key){
    TN* successor = NULL;
    while(root != NULL){
        if(key >= root->val){
            successor = root;
...
int main(){
    ...
    // TN* node = findInorderSuccessor(root, 10);
    TN* node = findInorderSuccessor_(root, 10);
    cout<<node<<" "<<node->val;
